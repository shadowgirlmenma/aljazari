...
const StrokeText = ({
  text = DEFAULT_TEXT,
  strokeColor = '#A78BFA',
  /* ملاحظة مراجعة 26/09/2026: كانت أبيض ثابت (#F8FAFC) دايماً — بالوضع
     الفاتح يصير نص أبيض فوق خلفية فاتحة (يختفي بعد أنيميشن التعبئة).
     var(--rt-white) نفس متغير باقي نصوص الموقع، يتبدّل تلقائياً مع الثيم. */
  ...
  // النصوص العربية لازم تترسم كسطر واحد متصل (مو حرف-حرف) حتى ما تنكسر حروفها
  // المتصلة ولا ينعكس ترتيبها بصرياً — لأنه SVG ما يعمل shaping صح لو قسمناها.
  const isRtl = useMemo(() => /[؀-ۿݐ-ݿࢠ-ࣿﭐ-﷿ﹰ-﻿]/.test(String(text ?? '')), [text]);
  const characters = useMemo(() => (isRtl ? [String(text ?? '')] : Array.from(String(text ?? ''))), [text, isRtl]);
  // بالعربي التسبان وحدة تغطي الكلمة كاملة، فطول المسار أكبر بمرات — نكبّر dash حسب عدد الأحرف
  const dash = isRtl
    ...
    >
      ...
      >
        ...
        <text
          ...
          stroke="none"
          direction={isRtl ? 'rtl' : 'ltr'}
          style={{ ...fontStyle, fill: fillColor }}
