...
export default function TiltedCard({
  ...
  scaleOnHover = 1.1, rotateAmplitude = 14, showMobileWarning = false, showTooltip = true,
  overlayContent = null, displayOverlayContent = false, placeholder = null
}) {
  ...
  function handleMouse(e) {
    ...
    rotateFigcaption.set(-(offsetY - lastY) * 0.6); setLastY(offsetY);
  }
  ...
  return (
    <figure ref={ref} className="tilted-card-figure" style={{ height: containerHeight, width: containerWidth }}
      ...
      {showMobileWarning && <div className="tilted-card-mobile-alert">Desktop only effect.</div>}
      <motion.div className="tilted-card-inner" style={{ width: imageWidth, height: imageHeight, rotateX, rotateY, scale }}>
        {imageSrc ? (
          ...
          <div className="tilted-card-img tilted-card-placeholder" style={{ width: imageWidth, height: imageHeight }}>{placeholder}</div>
        )}
        {displayOverlayContent && overlayContent && <motion.div className="tilted-card-overlay">{overlayContent}</motion.div>}
      </motion.div>
      {showTooltip && captionText && (
        <motion.figcaption className="tilted-card-caption" style={{ x, y, opacity, rotate: rotateFigcaption }}>{captionText}</motion.figcaption>
      )}
    </figure>
  );
}
