...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    let stageH = 0;
    let running = false;
    /* أعلى نسبة توسّع وصلتها الصورة — ما ترجع تنزل لتحت لو رجعنا نسكرول لفوق،
       حتى الصورة تضل ظاهرة بالكامل بعد ما توصل لآخر توسّعها */
    let maxReached = 0;
    const measure = () => {
      const c = propsRef.current;
    ...
    const onScroll = () => {
      maxReached = Math.max(maxReached, readProgress());
      target = maxReached;
    ...
    >
      <div ref={trackRef} className="relative w-full">
        <div ref={stageRef} className="sticky top-0 w-full overflow-hidden [--se-title-size:4rem]">
          ...
          >
            {media}
            {/* تدرّج تعتيم بلون خلفية الموقع البنفسجية الداكنة (#0a0414) بدل الأسود الصرف
                حتى ما يصير أي اختلاف عن باقي خلفيات الموقع */}
            <div
              ref={scrimRef}
              className="absolute inset-0 opacity-0 pointer-events-none bg-[linear-gradient(to_top,rgba(10,4,20,0.85),rgba(10,4,20,0.12)_45%,rgba(10,4,20,0.4))]"
            />
            {children ? (
              <div
                ref={overlayRef}
          ...
          {title ? (
            <div
              ref={titleRef}
              className="absolute inset-0 flex items-center justify-center m-0 px-[6%] text-center font-bold leading-none tracking-[-0.03em] text-white [font-size:var(--se-title-size)] [text-shadow:0_2px_24px_rgba(10,4,20,0.6)] pointer-events-none [will-change:opacity,transform]"
            >
              {title}
            </div>
          ) : null}
